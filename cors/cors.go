...
type Options struct {
	...
	HttpHandlerFunc func(w http.ResponseWriter,r *http.Request,name string)
}
...
func New(options Options) *Cors {
	c := &Cors{
		...
		HttpHandlerFunc  : options.HttpHandlerFunc,
	}
	if options.Debug {
...
func (c *Cors) Handler(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Println("[lgh:]","get request",r.URL.Path,r.URL.Query())
		if r.Method == "OPTIONS" && r.Header.Get("Access-Control-Request-Method") != "" {
			...
			if r.Method == "GET" && !strings.Contains(r.URL.Path,"rpc") {
				...
				if len(value) > 0 && value[0] != "" {
					if c.HttpHandlerFunc != nil {
						c.HttpHandlerFunc(w,r,value[0])
						return
					}
				}
			}
